...
const admin=addUser("Food Bite Admin","9999999999","admin123","admin");
...
app.post("/api/orders",auth,role("customer"),(req,res)=>{
 const {shopId,items,address}=req.body||{}; if(!shopId||!items?.length||!address)return res.status(400).json({error:"Shop/items/address required"});
 let total=0, checked=[], get=db.prepare("SELECT * FROM menu WHERE id=? AND shop_id=?");
 try{for(const x of items){const m=get.get(x.menuId,shopId);if(!m)throw Error("Invalid item");total+=m.price*x.qty;checked.push({...m,qty:x.qty})}}
 catch(e){return res.status(400).json({error:e.message})}
 const r=db.prepare("INSERT INTO orders(user_id,shop_id,total,address) VALUES(?,?,?,?)").run(req.user.id,shopId,total,address);
 const ins=db.prepare("INSERT INTO order_items(order_id,menu_id,name,qty,price) VALUES(?,?,?,?,?)");checked.forEach(x=>ins.run(r.lastInsertRowid,x.id,x.name,x.qty,x.price));
 res.json({orderId:r.lastInsertRowid,total,status:"PLACED",payment_status:"PENDING"});
});
...
app.patch("/api/admin/orders/:id",auth,role("admin"),(req,res)=>{db.prepare("UPDATE orders SET status=? WHERE id=?").run(req.body.status,req.params.id);res.json({ok:true})});
...
app.patch("/api/partner/orders/:id",auth,role("restaurant"),(req,res)=>{
 ...
 db.prepare("UPDATE orders SET status=? WHERE id=? AND shop_id=?").run(req.body.status,req.params.id,s.id);res.json({ok:true})
});

app.get("/api/delivery/orders",auth,role("delivery"),(req,res)=>res.json(db.prepare(`SELECT o.*,s.name shop_name,s.address shop_address,u.name customer FROM orders o JOIN shops s ON s.id=o.shop_id JOIN users u ON u.id=o.user_id WHERE o.status IN ('READY','ON_THE_WAY') OR o.delivery_id=? ORDER BY o.id DESC`).all(req.user.id)));
app.patch("/api/delivery/orders/:id",auth,role("delivery"),(req,res)=>{db.prepare("UPDATE orders SET status=?,delivery_id=? WHERE id=?").run(req.body.status,req.user.id,req.params.id);res.json({ok:true})});

app.get("/{*splat}",(req,res)=>res.sendFile(path.join(__dirname,"public","index.html")));
app.listen(process.env.PORT||3000,()=>console.log("Food Bite running"));
