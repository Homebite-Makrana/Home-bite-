...
app.post("/api/payment/confirm",auth,role("customer"),async(req,res)=>{
  try{
    ...
    res.json({ok:true,orderId:order.id,payment_status:"PAID",payment_id:paid.id});
  }catch(e){
...
app.patch("/api/partner/orders/:id",auth,role("restaurant"),(req,res)=>{
 ...
 db.prepare("UPDATE orders SET status=? WHERE id=? AND shop_id=?").run(req.body.status,req.params.id,s.id);notifyOrderStatus(req.params.id,req.body.status);res.json({ok:true})
});
